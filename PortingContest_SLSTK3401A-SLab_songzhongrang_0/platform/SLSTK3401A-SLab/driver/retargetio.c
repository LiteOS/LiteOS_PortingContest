...
extern int USART_RETARGET_ReadChar(void);
extern int USART_RETARGET_WriteChar(char c);
...
int _read(int file, char *ptr, int len)
{
  ...
  while (len--)
  {
    if ((c = USART_RETARGET_ReadChar()) != -1)
    {
      *ptr++ = c;
      rxCount++;
    }
...
static int TxBuf(uint8_t *buffer, int nbytes)
{
  ...
  for (i = 0; i < nbytes; i++)
  {
    USART_RETARGET_WriteChar(*buffer++);
  }
  return nbytes;
}
...
{
  return(USART_RETARGET_WriteChar(ch));
}

int __getchar(void)
{
  return(USART_RETARGET_ReadChar());
}
